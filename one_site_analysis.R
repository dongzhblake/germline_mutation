...
# make genomic location SI plots
#  the build here is hg19/37. 
make_geno_plotz<-function(ces_obj,stage_data,gene,group1,group2,start,end,i){
  allsnvs=data.frame(analysis$mutations$snv)
  variants_ref_gene=stage_data[stage_data$group==group1 & stage_data$variant%in%strsplit(stage1_info[stage1_info$gene==gene,]$all_variants,",")[[1]],]
  allsnvs1=allsnvs[allsnvs$assoc_aa_mut %in% variants_ref_gene$variant,]
  names(allsnvs1)[6] = "variant"
  variants_ref_gene=merge(variants_ref_gene,allsnvs1,by="variant")
  plot(variants_ref_gene$selection_intensity~variants_ref_gene$pos,type="h",lwd=1,col="red",xlim=c(start,end),xlab="gemonic position",ylab="selection intensity",main=paste0("genomic position and SI of SNPs in gene ",gene,", site ",i))
  variants_alt_gene=stage_data[stage_data$group==group2 & stage_data$variant%in%strsplit(stage1_info[stage1_info$gene==gene,]$all_variants,",")[[1]],]
  allsnvs1=allsnvs[allsnvs$assoc_aa_mut %in% variants_ref_gene$variant,]
  names(allsnvs1)[6] = "variant"
  variants_alt_gene=merge(variants_alt_gene,allsnvs1,by="variant")
  plot(variants_alt_gene$selection_intensity~variants_alt_gene$pos,type="h",lwd=1,col="blue",xlim=c(start,end),xlab="gemonic position",ylab="selection intensity",main=paste0("genomic position and SI of SNPs in gene ",gene,", site ",i))
  
  plot(variants_ref_gene$selection_intensity~variants_ref_gene$pos,type="h",lwd=1,col="red",xlim=c(start,end),xlab="gemonic position",ylab="selection intensity",main=paste0("genomic position and SI of SNPs in gene ",gene,", site ",i))
  lines(variants_alt_gene$selection_intensity~variants_alt_gene$pos,type="h",lwd=1,col="blue")
  lines(rep(50000,length(exome_start))~exome_start,type="h",lwd=0.5,col="dark green")
  lines(rep(50000,length(exome_start))~exome_end,type="h",lwd=0.5,col="purple")
...
TP53 <- read.csv("~/Desktop/TP53.csv",stringsAsFactors = F)
TP53=TP53[order(TP53[,2]),]
exome_start=c(TP53$start)
exome_end=c(TP53$end)


make_geno_plotz(analysis,stage_data,"TP53","homo_ref","any_alt",7571720,7590868,3)
...
names(allsnvs1)[6] = "variant"
variants_ref_gene=merge(variants_ref_gene,allsnvs1,by="variant")
plot(variants_ref_gene$selection_intensity~variants_ref_gene$pos,type="h",lwd=2,col="red")
variants_ref_gene$coding=apply(variants_ref_gene,1,FUN=function(x){max((x[7]>=exome_start)*(x[7]<=exome_end))})


variants_alt_gene=stage_data[stage_data$group=="any_alt" & stage_data$variant%in%strsplit(stage1_info[stage1_info$gene==gene,]$all_variants,",")[[1]],]
allsnvs1=allsnvs[allsnvs$assoc_aa_mut %in% variants_ref_gene$variant,]
names(allsnvs1)[6] = "variant"
variants_alt_gene=merge(variants_alt_gene,allsnvs1,by="variant")
lines(variants_alt_gene$selection_intensity~variants_alt_gene$pos,type="h",lwd=2,col="blue")
